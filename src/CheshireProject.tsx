...
export function CheshireProject() {
  return <main className="tree-page">
    <header className="tree-header">
      <Link to="/" className="tree-brand"><Mark /><span>thrivoli</span></Link>
      <div><p>CHESHIRE FITNESS ZONE</p><h1>Profitability Map</h1></div>
      <p className="tree-purpose">Revenue − direct costs − shared overhead = profitability</p>
    </header>

    <section className="tree-canvas" aria-label="Cheshire profitability model">
      <div className="tree-root">Cheshire <strong>Profitability</strong></div>
      <div className="tree-connector" aria-hidden="true" />
      <div className="tree-columns">
        {segments.map((segment) => <article className={`tree-card ${segment.tone}`} key={segment.name}>
          <header><span className="tree-number">{segment.number}</span><segment.icon size={18} aria-hidden="true" /><h2>{segment.name}</h2></header>
      ...
      <section className="tree-bottom">
        <div className="shared-costs"><h2>Shared overhead <small>allocated using a documented driver</small></h2><Tags items={["Administrative labor", "Billing & software", "Insurance & professional fees", "Corporate G&A"]} /></div>
        <div className="profit-output"><span>Dashboard views</span><strong>Location · Contract · Therapist · Service line</strong><small>Revenue · Contribution margin · Operating profit · Utilization · Break-even</small></div>
      </section>
    </section>
  </main>;
}
